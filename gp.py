...
parser.add_argument('--label_num', type=int, default=6, help='number of ner labels')
parser.add_argument('--per_save', type=int, default=int(np.ceil(6932 / 2)), help='save model per num')
parser.add_argument('--check', type=str, default='model/gp', help='The path where model saved')
...
class GP(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        super(GP, self).build(input_shape)
    ...
    def call(self, inputs, **kwargs):
        ...
        tri = tf.sequence_mask(tf.range(1, N + 1), maxlen=N)
        tri = tf.tile(tf.expand_dims(tri, 0), [B, 1, 1])

        val = tf.logical_and(val, tri)

        # B*N*N
        loss = loss * tf.cast(val, tf.float32)

        # 1
        seqlenf = tf.cast(seqlen, tf.float32)
        seqlen2sum = tf.reduce_sum(seqlenf * (seqlenf + 1.) / 2.0)
...
def querycheck(predict):
    sys.stdout.write('\n')
    sys.stdout.flush()
    labels = ['NONE', "TREATMENT", "BODY", "SIGNS", "CHECK", "DISEASE"]
...
def train_step(data, model, optimizerbert, optimizergp):
    with tf.GradientTape(persistent=True) as tape:
        _, tp, tn, fp, loss, accuracy = model(data, training=True)

    gp_trainable_variables = [v for v in model.trainable_variables if v.name.startswith("gp")]
    bert_trainable_variables = [v for v in model.trainable_variables if not v.name.startswith("gp")]

    gradientsgp = tape.gradient(loss, gp_trainable_variables)
    optimizergp.apply_gradients(zip(gradientsgp, gp_trainable_variables))
...
class USER:
    ...
    def build_model(self, summary=True):
        ...
        predict = GP(name="gp")(inputs=(sequence_split, span, seqlen))
    ...
    def train(self):
        ...
        if params.mode == 'train1':
            model.load_weights(params.check + '/gp.h5')
        ...
        optimizergp = AdamWeightDecay(learning_rate=warmup_schedule,
                                         weight_decay_rate=0.01,
                                         epsilon=1.0e-6,
                                         global_clipnorm=1.0,
        ...
        for epoch in range(params.epochs):
            ...
            for batch, data in enumerate(train_dataloader):
                tp_, tn_, fp_, loss_, accuracy_ = train_step(data, model, optimizerbert, optimizergp)
            ...
            if F1_val > F1_max:
                model.save_weights(params.check + '/gp.h5')
                F1_max = F1_val
    ...
    def plothistory(self):
        ...
        plt.tight_layout()
        plt.savefig("gp_PRF.jpg", dpi=500, bbox_inches="tight")
